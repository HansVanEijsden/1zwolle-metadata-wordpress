...
Resolution order: **special song** (configurable, currently disabled for 1Zwolle until its special-song plugin is live) → **iTunes** (artist + title matching with compilation/remix penalties, ≥ 120s track) → **show avatar** (from the already-cached WordPress metadata) → **fallback image**.
