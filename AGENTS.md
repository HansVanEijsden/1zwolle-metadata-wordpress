...
CI (`.github/workflows/ci.yml`) runs `go mod verify`, `go vet`, `gofmt` check, tests, and build — keep these green.

## Architecture

Standard library only. Module path: `1zwolle-metadata-wordpress`.
...
- `tests/parser_test.go` — table-driven tests for the parser.
...
| `COVERART_HUB_URL` | — | Hub base URL to push results to (e.g. `http://172.21.0.38:9000`) |
